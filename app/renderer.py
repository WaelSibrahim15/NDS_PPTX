...
def _font(family: str, bold: bool, italic: bool, px: int, custom: str = "") -> ImageFont.FreeTypeFont:
    """custom: a template's font file for this style (tried first)."""
    if custom:
        try:
            return ImageFont.truetype(custom, px)
        except OSError:
            pass
...
def _font_file(font_files: dict, family: str, bold: bool, italic: bool) -> tuple:
    """(path, fake_bold) for a template font: the exact style if the template has
    it, else its regular file drawn with a stroke for bold."""
    base = font_files.get(family)
    styled = font_files.get(f"{family}_bold" if bold else f"{family}_italic" if italic else family)
    if styled:
        return str(styled), False
    if base:
        return str(base), bold
    return "", False


def _layout_para(draw, para: "design.Para", max_w: int, font_files: dict):
    """Wrap a paragraph of styled runs. Returns [(line_height, [(x, text, font, color, stroke)])]."""
    words = []  # (text, font, color, trailing_space, size, stroke)
    for run in para.runs:
        path, fake_bold = _font_file(font_files, run.font, run.bold, run.italic)
        px = max(1, int(run.size * PT))
        font = _font(run.font, run.bold, run.italic, px, path)
        stroke = max(1, px // 28) if fake_bold else 0
        parts = run.text.split(" ")
        for i, w in enumerate(parts):
            if w == "" and i not in (0, len(parts) - 1):
                continue
            words.append((w, font, _rgb(run.color), i < len(parts) - 1, run.size, stroke))
    lines, cur, x = [], [], 0
    size_max = 0
    space_cache = {}
    for text, font, color, space, size, stroke in words:
        ...
        if cur and x + w > max_w and text:
            ...
            cur, x, size_max = [], 0, 0
        if text or cur:
            cur.append((x, text, font, color, stroke))
...
def _draw_text(draw, el: "design.Text", font_files: dict):
    x0, y0, bw, bh = _px(el.x), _px(el.y), _px(el.w), _px(el.h)
    blocks = []
    total = 0
    for para in el.paras:
        lines = _layout_para(draw, para, bw, font_files)
    ...
    for para, lines, gap in blocks:
        for lh, items in lines:
            ...
            for x, text, font, color, stroke in items:
...
def _draw_element(img: Image.Image, draw: ImageDraw.ImageDraw, el, font_files: dict):
...
def render_slide(plan: DeckPlan, index: int, template="niq") -> Image.Image:
    """Render one slide at full 1920×1080 resolution (template: id or theme dict)."""
    ...
    for el in scene.elements:
        _draw_element(img, draw, el, scene.font_files)
    return img.resize((W, H), Image.Resampling.LANCZOS)


def render_slide_preview(plan: DeckPlan, index: int, template="niq",
...
def render_frames(plan: DeckPlan, out_dir: Path, template="niq") -> List[Path]:
